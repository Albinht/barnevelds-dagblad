...
export default function ShowGrid({ articles }: ShowGridProps) {
  ...
  return (
    <section className="mb-12">
      <div className="border-b-4 border-brand-blue mb-6">
        <h2 className="text-2xl font-bold text-brand-blue uppercase mb-2">SHOW</h2>
      </div>

      {/* Grid Section */}
      {gridArticles.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 h-auto md:h-[400px] mb-6">
          {/* Main Square Article */}
          {gridArticles[0] && (
            <Link
              href={`/artikel/${gridArticles[0].slug}`}
              className="md:col-span-1 relative group overflow-hidden h-[300px] md:h-auto"
            >
              <Image
              ...
              {/* Entertainment Badge */}
              <div className="absolute top-4 left-4 bg-purple-600 text-white text-xs font-bold px-3 py-1 rounded uppercase">
                ENTERTAINMENT
              </div>
              
              {/* Event Date */}
              <div className="absolute top-4 right-4 bg-white/90 text-black text-xs font-bold px-3 py-2 rounded">
                15 SEP
              </div>
              
              <div className="absolute bottom-0 left-0 right-0 p-4">
                <div className="text-xs text-white/80 uppercase font-semibold mb-2">
                  {gridArticles[0].category} • THEATER
                </div>
                ...
                <div className="flex items-center text-white/70 text-xs">
                  ...
                  <span>Concertgebouw</span>
                </div>
              </div>
            </Link>
          )}

          {/* Right Column - Portrait Articles */}
          <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-2">
            {gridArticles[1] && (
              <Link
                href={`/artikel/${gridArticles[1].slug}`}
                className="relative group overflow-hidden h-[200px] md:h-auto"
              >
                <Image
                ...
                {/* Cinema Badge */}
                <div className="absolute top-2 left-2 bg-red-600 text-white text-xs font-bold px-2 py-1 rounded uppercase">
                  CINEMA
                </div>
            ...
            {gridArticles[2] && (
              ...
              >
                ...
                {/* Music Badge */}
                <div className="absolute bottom-12 left-2 bg-indigo-600 text-white text-xs font-bold px-2 py-1 rounded uppercase">
                  MUZIEK
                </div>
                
                {/* Ticket Link */}
                <div className="absolute bottom-12 right-2 bg-green-600 text-white text-xs font-bold px-2 py-1 rounded">
                  TICKETS
                </div>
          ...
          {listArticles.slice(0, 3).map((article) => (
            ...
            >
              <div className="w-24 h-16 md:w-40 md:h-24 flex-shrink-0 relative overflow-hidden rounded">
                ...
                {/* Show Badge */}
                <div className="absolute top-1 right-1 bg-purple-600 text-white text-xs font-bold px-1 py-0.5 rounded uppercase">
                  SHOW
                </div>
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-xs md:text-sm text-purple-600 font-semibold mb-1 md:mb-2 uppercase">
                  SHOW • {article.publishedAt}
                </div>
