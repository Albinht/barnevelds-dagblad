...
import { getAdverterenEmailTemplates } from '@/lib/email-templates'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { adminEmail, confirmationEmail } = getAdverterenEmailTemplates(body)
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'Bedankt voor uw aanvraag. We nemen binnen 24 uur contact met u op voor een offerte.'
      },
      { status: 200 }
    )
  } catch (error) {
